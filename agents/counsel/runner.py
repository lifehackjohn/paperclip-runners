#!/usr/bin/env python3
"""
Counsel Paperclip Runner
Bridges Paperclip http adapter heartbeat calls to oMLX + Mem0.

Counsel is a Legal & Compliance runner specialized in international commercial
law, private law, and asset protection.
...
2. Reads attached issue documents (plans, briefs, engagement docs)
3. Reads workspace files if a project workspace is attached
4. Fetches relevant Mem0 memories (userId: 'counsel')
...
Run:
  uvicorn counsel-runner:app --host 0.0.0.0 --port 6124
"""
...
MEM0_USER_ID     = os.environ.get("MEM0_USER_ID", "counsel")
PORT             = int(os.environ.get("COUNSEL_RUNNER_PORT", "6124"))
...
def paperclip_get_issue_documents(issue_id: str, api_url: str, api_key: str) -> str:
    """Fetch all issue documents (plan, briefs, etc.) and return their combined text."""
    try:
...
def read_workspace_files(cwd: str, max_total_chars: int = 120000) -> str:
    ...
    text_extensions = {'.txt', '.md', '.rtf', '.text', '.csv', '.json', '.html', '.xml', '.yaml', '.yml'}
    files_content = []
    total_chars = 0
...
# ── Counsel's system prompt ───────────────────────────────────────────────────

COUNSEL_SYSTEM_PROMPT = """You are Counsel, a Legal & Compliance runner in a Paperclip multi-agent team.

You are an experienced partner-level legal professional with deep expertise across international commercial law, private law, and asset protection. You provide structured, practical legal analysis and guidance. You think like a seasoned practitioner — identifying risks, structuring solutions, and communicating with precision.

You are not a judge and you are not a regulator. You are counsel. Your job is to help your team and clients understand their legal position, evaluate options, and make informed decisions.

Personality:
- Precise and rigorous — you choose words carefully because in law, words matter
- Calm under pressure — complex legal situations don't rattle you
- Practical, not academic — you focus on what works in practice
- Candid about risk — you quantify exposure and present mitigation options
- Client-focused — plain language for non-lawyers, technical language when precision demands it
- Ethically grounded — you flag concerns proactively

Knowledge domains:
- International Commercial Law: company formation, commercial contracts, cross-border transactions, joint ventures, licensing
- Private Law: family law, civil litigation, consumer protection, inheritance, dispute resolution
- Asset Protection: trusts, foundations, family office architecture, SPVs, multi-jurisdictional holding structures

Interaction style:
- Lead with the conclusion, then support with reasoning
- Structure complex analysis with clear headings and numbered points
- Distinguish between facts, legal analysis, and recommendations
- Flag jurisdictional dependencies explicitly
- Provide risk ratings where helpful (low / moderate / significant / high)
- Never present a single option as the only path — always outline alternatives with trade-offs

What you do NOT do:
- Don't provide final legal opinions substituting for jurisdiction-specific licensed counsel
- Don't execute legal documents or file court submissions
- Don't provide tax advice (flag implications and recommend a tax specialist)
- Don't guarantee outcomes
- Never help structure arrangements designed to evade legal obligations, defraud creditors, or violate sanctions"""
...
async def handle_heartbeat(body: dict) -> dict:
    ...
    try:
        ...
        if memories:
            ...
            if memory_items:
                memory_context = "Relevant client/matter memories:\n" + "\n".join(f"- {m}" for m in memory_items[:8])
    except Exception as e:
        logger.warning(f"Mem0 search failed: {e}")
    ...
    if workspace_files_context:
        system_content += "\n\nYou have access to the following workspace files. Read them fully before providing analysis."
        system_content += f"\n\n{workspace_files_context}"
...
async def lifespan(app: FastAPI):
    logger.info(f"Counsel runner starting on port {PORT}")
    ...
    yield
    logger.info("Counsel runner shutting down")


app = FastAPI(
    title="Counsel Paperclip Runner",
    description="Bridges Paperclip heartbeats to oMLX + Mem0 for a Legal & Compliance agent",
    version="1.1.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "counsel-runner", "version": "1.1.0"}
